<?php
/**
 * Web-accessible script to add 10 sample quizzes to the database
 * Access this page once to populate sample data
...
$page_title = 'Add Sample Quizzes';
require_login();
// Allow any logged-in user to run this (for setup purposes)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_quizzes'])) {
    ...
    if (!$user) {
        ...
        if ($user_id) {
            ...
            $message .= "Created user: $username<br>";
        } else {
            $error = "Failed to create user";
        }
    } else {
        $user_id = $user['id'];
        $message .= "Using existing user: $username (ID: $user_id)<br>";
    }
    ...
    if (!$error) {
        ...
        foreach ($sample_quizzes as $quiz_data) {
            ...
            if ($quiz_id) {
                ...
                $added_count++;
                $message .= "Added quiz: {$quiz_data['title']} (ID: $quiz_id)<br>";
            } else {
                $error .= "Failed to add quiz: {$quiz_data['title']}<br>";
            }
        }
        
        if ($added_count > 0) {
            $message = "<strong>Success!</strong> Added $added_count quizzes.<br><br>" . $message;
        }
    }
}
?>
<div class="container">
    <h1>Add Sample Quizzes</h1>
    
    <div class="card">
        <div class="card-header">
            <h2>Sample Quiz Generator</h2>
        </div>
        ...
        <p>This will add 10 sample quizzes to the database for the user <strong>jpazi</strong> (juan_porferio_napiza@dlsu.edu.ph).</p>
        <p style="color: #FF3366; font-weight: 700;">⚠️ WARNING: This will create quizzes if they don't exist. Run this only once.</p>
        
        <form method="POST" action="">
            <input type="hidden" name="add_quizzes" value="1">
            <button type="submit" class="btn btn-primary btn-large">Add 10 Sample Quizzes</button>
        </form>
    </div>
